...
func (r *UserRepository) Get(ctx context.Context, userID domain.UserID) (domain.User, error) {
	row := r.db.QueryRowContext(ctx, `
		SELECT telegram_id, username, last_activity_at, plan, trial_ends_at, discount_percent, created_at, updated_at
		FROM users
...
func (r *UserRepository) Upsert(ctx context.Context, user domain.User) (domain.User, error) {
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO users (
			telegram_id, username, last_activity_at, plan, trial_ends_at, discount_percent, created_at, updated_at
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
func scanUser(row *sql.Row) (domain.User, error) {
	...
	err := row.Scan(
		&u.TelegramID, &username, &activity, &u.Plan, &trialEnds, &discount, &createdAt, &updatedAt,
	)
	if err != nil {
